...
# The client will be initialized after the user provides their API key in the UI.
client = None
...
# --- Core Functions ---
def generate_transcript_and_prompt(audio_input, text_input, use_internet): #api_key, 
    global client # Allow this function to modify the global client variable
    ...
    try:
        ...
        # Catches authentication errors and other initialization issues.
        raise gr.Error(f"Failed to initialize the client with the provided API Key. Please check your key. Error details: {e}")
...
with gr.Blocks(theme='JohnSmith9982/small_and_pretty', title="Gemini Podcast Studio") as demo: #gr.themes.Soft js = on_start_js, 
    ...
    with gr.Row():
        ...
        with gr.Column(scale=10):
            # gr.HTML("<div style='margin-top: 0px;'></div>")
            gr.Markdown("# 🎙️ RK Podcast Studio"), 
    ...
    progress_bar = gr.HTML("""
        <div style="width: 100%; background-color: #f0f0f0; border-radius: 10px; margin: 0px 0;">
    ...
    with gr.Column(visible=True) as step1_container:
        ...
        #         label="🔑 Enter Your Google API Key to Begin", 
        #         type="password",
    ...
    with gr.Column(visible=False) as step2_container:
        # Group Radio and speaker inputs in a single row for horizontal alignment
        with gr.Row():
            # Radio selector column
            with gr.Column(scale=1):  # Smaller width
                num_speakers_selector = gr.Radio(["1", "2"], label="Number of Speakers", value="2")

            # Speaker input fields
            with gr.Column(scale=3):  # Bigger width for inputs
                with gr.Row():
                    with gr.Group():
    ...
    generate_button.click(
        ...
        inputs=[audio_prompt_input, text_prompt_input, use_internet_checkbox], #api_key_input, 
        outputs=[step_state, transcript_display, full_transcript_state]
        ) # processing_status, improved_prompt_display,
    ...
    generate_audio_button.click(
        ...
        outputs=[step_state, podcast_output]
    )
    ...
    restart_button.click(
        fn=restart_process,
        outputs=[step_state, transcript_display, full_transcript_state, podcast_output, audio_prompt_input, text_prompt_input, final_transcript_step3_display, use_internet_checkbox]
    ) # processing_status, audio_status, improved_prompt_display,
